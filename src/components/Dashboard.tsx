...
export default function Dashboard() {
  ...
  const handleTimeBlockUpdate = (
    dayId: string,
    blockId: string,
    data: { done?: boolean; activity?: string; rating?: BlockRating | null },
  ) => {
    setDays((prev) =>
      prev.map((day) =>
        day.id === dayId
  ...
  const toggleDayCompleted = async (day: Day) => {
    try {
      ...
      if (res.ok) {
        setDays((prev) =>
          prev.map((d) =>
            d.id === day.id ? { ...d, completed: !d.completed } : d,
          ),
        );
      }
    } catch (error) {
      ...
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4 overflow-hidden relative">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(circle, #2383E2 0%, transparent 70%)",
              top: "-10%",
              left: "-10%",
            }}
          />
          ...
          <div
            className="absolute w-2 h-2 bg-blue-400/40 rounded-full blur-[2px] animate-particle-1"
            style={{ top: "20%", left: "30%" }}
          />
          <div
            className="absolute w-1.5 h-1.5 bg-purple-400/40 rounded-full blur-[2px] animate-particle-2"
            style={{ top: "60%", left: "70%" }}
          />
          <div
            className="absolute w-2 h-2 bg-cyan-400/30 rounded-full blur-[2px] animate-particle-3"
            style={{ top: "80%", left: "20%" }}
          />
          <div
            className="absolute w-1 h-1 bg-pink-400/50 rounded-full blur-[1px] animate-particle-4"
            ...
            <div className="relative">
              ...
              <form onSubmit={handleAuth} className="space-y-5">
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs text-white/50 mb-2 uppercase tracking-wider font-medium">
                      Username
                    </label>
                    <input
                      type="text"
        ...
        <header className="fixed top-0 left-0 right-0 h-14 bg-[var(--bg-overlay)] backdrop-blur-md border-b border-[var(--border-primary)] z-50 transition-all duration-300">
          <div className="max-w-3xl mx-auto h-full px-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h1 className="text-sm font-medium text-[var(--text-secondary)] hidden sm:block">
            ...
            <div className="flex items-center gap-2 sm:gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="text-xs bg-[var(--bg-tertiary)] hover:bg-[var(--bg-secondary)] text-[var(--text-secondary)] px-3 py-1.5 rounded transition-all whitespace-nowrap"
                >
                  Sign Out
                </button>
              </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20">
              <p className="text-[var(--text-secondary)]">
                No days yet. Create your first day!
              </p>
            </div>
          ) : viewMode === "list" ? (
            <div className="space-y-4">
              {days.map((day) => (
                <DayCard
                  ...
                  onUpdateBlock={(blockId, data) =>
                    handleTimeBlockUpdate(day.id, blockId, data)
                  }
              ...
              <div className="mt-4 space-y-4 animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div>
                  ...
                  <select
                    value={sleepQuality}
                    onChange={(e) =>
                      setSleepQuality(e.target.value as SleepQuality | "")
                    }
