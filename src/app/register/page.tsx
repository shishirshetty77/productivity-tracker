  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (result?.error) {
        if (typeof result.error === 'string') {
            ...
            // Handle field errors if needed, simpler for now
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#191919]">
      <div className="w-full max-w-md p-8 bg-[#202020] rounded-2xl border border-[#373737]">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-[#aaa] mb-2">Confirm Password</label>
            <input
        ...
        <p className="mt-6 text-center text-sm text-[#aaa]">
          ...
          <Link href="/login" className="text-[#2383e2] hover:underline">
            Login
          </Link>
        </p>
      </div>
